...
/* MD5Hashing - RSA Data Security, Inc., MD5 message-digest algorithm
...
unsigned char result_char[ 1024 ];
...
/*
 ...
 inside the variable MD5Digest[16], an unsigned char[]. */
void generateMD5(unsigned char strInputString[], unsigned int len)
{
	MD5_CTX ctx;
	MD5Init(&ctx);
...
char printMD5( unsigned char *data , unsigned int length , int count )
{
  char tmp[ 16 ];
  for(int looker = 0 ; looker < length ; looker++){
    sprintf(tmp, "%.2X", data[ looker ]);
  }
  return tmp[ 0 ];
}
